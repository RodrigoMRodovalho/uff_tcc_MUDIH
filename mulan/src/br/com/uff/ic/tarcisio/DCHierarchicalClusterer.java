 ...
 */
public class DCHierarchicalClusterer extends AbstractClusterer implements
  OptionHandler, CapabilitiesHandler, Drawable {
	
	public boolean recursive=false;
  private static final long serialVersionUID = 1L;
  //inserindo a lista de r�tulos
  ...
  void doLinkClustering(int nClusters, Vector<Integer>[] nClusterID, Node[] clusterNodes) {
	  //nClusters tem o valor= numero de instancias,  nClusterID � um vetor com a ID de cada instancia
	  //clusterNodes � um vetor de node com tamanho igual ao numero de instancias, com suas c�lulas vazias
    int nInstances = m_instances.numInstances();
    ...
    while (nClusters > m_nNumClusters) {
      ...
      nClusters--;
      if(nClusters==3 && recursive){
    	  int k=-1;
    	  for (int i = 0; i < nInstances; i++) {
    		  if(nClusterID[i].size() == 1){
    			  if(k!=-1){
    				  double dist=getDistance0(nClusterID[k],nClusterID[i]);
    				  merge(k, i, dist, dist, nClusterID, clusterNodes); 
    			      //m_clusters=clusterNodes;
    				  k=-2;
    				  break;
    			  }else k=i;
      ...
      if(nClusters == m_nNumClusters){
    	  for(int i=0;i<nClusterID.length;i++){
			if(nClusterID[i].size()==1 && m_nNumClusters>1){
				  m_nNumClusters--;
				  break;
			  }
    	  }
  ...
  public static void main(String[] argv) {
    runClusterer(new DCHierarchicalClusterer(), argv);
  }
